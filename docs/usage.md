...
The default source is [[private]](https://github.com/evgeniyarbatov/[private]). `make random`, `make dtwselect`, and `make art` clone or update it under `~/Documents/data/gpx-art/[private]`.
...
| `make [private]` | Clone or update `[private]` into the data directory |
...
Variables: `SOURCE_DIR` (default `$(DATA_DIR)/[private]/data/parquet`), `NUMBER_OF_GPX` (default `20`), `GPX_DIR`, `IMAGES_DIR`.
