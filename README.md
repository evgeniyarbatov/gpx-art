...
This project takes GPS tracks (from `[private]` parquet by default, or local GPX), renders each route in multiple visual styles, and saves PNG outputs.
...
This clones or updates [[private]](https://github.com/evgeniyarbatov/[private]), samples tracks from its per-city parquet files, and writes styled PNGs to `images/`.

Generated selections and rendered art live under `~/Documents/data/gpx-art/` by default (`[private]/`, `gpx/`, `images/`), not in the repo. Override with `DATA_ROOT=/other/root` (keeps the `gpx-art` subfolder) or `DATA_DIR=/exact/path`.
