...
def whisper(lons, lats):
    ...
    for _ in range(30):
        ...
        indices = sorted(
            random.sample(
                range(len(lons)), random.randint(len(lons) // 3, 2 * len(lons) // 3)
            )
        )
...
def radial(lons, lats):
    ...
    # Mark center
    ax.plot(center_lon, center_lat, "o", color=fg_color, markersize=4, alpha=0.8)
...
def field(lons, lats):
    ...
    for _ in range(num_lines):
        ...
        for step in range(30):
            ...
            distances = np.sqrt(
                (np.array(lons) - current_x) ** 2 + (np.array(lats) - current_y) ** 2
            )
            nearest_idx = np.argmin(distances)
        ...
        if norm > 0:
            ...
            for h in range(num_hatches):
                ...
                end_x = (
                    start_x
                    + length
                    * (perp_dx * np.cos(angle) + dx / norm * np.sin(angle))
                    * side
                )
                end_y = (
                    start_y
                    + length
                    * (perp_dy * np.cos(angle) + dy / norm * np.sin(angle))
                    * side
                )
...
def main(gpx_dir, images_dir):
    ...
    for name, gpx_path in get_files(gpx_dir):
        for style in sorted(STYLES.keys()):
            output_filename = os.path.join(images_dir, f"{style}-{name}.png")
